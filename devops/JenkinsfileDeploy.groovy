            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Verify ArgoCD startup/cleanup script authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Verify ArgoCD startup/cleanup script failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Verify ArgoCD startup/cleanup script deploying"
                ...
                slackSend color: "good",
                        message: "Verify ArgoCD startup/cleanup script deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Verify ArgoCD startup/cleanup script failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Verify ArgoCD startup/cleanup script shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Verify ArgoCD startup/cleanup script failed to shut down properly"
            throw err
        }
    }
}
